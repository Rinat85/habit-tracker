<script setup>
    ...
    const sortByNew = () => emit('sortByNew')
    const sortByOld = () => emit('sortByOld')
</script>
...
<template>
  <Menu as="div" class="relative inline-block p-6">
    <MenuButton class="inline-flex w-full justify-center gap-x-1.5 rounded-md text-white rounded-lg 
                                ...
                                shadow-indigo-500/40 active:scale-95 transition py-4 px-3 text-sm font-semibold 
    ...
    <transition enter-active-class="transition ease-out duration-100" enter-from-class="transform opacity-0 scale-95" enter-to-class="transform scale-100" leave-active-class="transition ease-in duration-75" leave-from-class="transform scale-100" leave-to-class="transform opacity-0 scale-95">
      <MenuItems class="absolute right-0 z-10 mt-2 w-46 origin-top-right divide-y divide-white/40 rounded-md bg-indigo-400 outline-1 -outline-offset-1 outline-white/10">
        <div class="py-1">
          <MenuItem v-slot="{ active }">
            <button @click="sortByNew" class="w-full text-start">
              <span :class="[
        ...
        <div class="py-1">
            <MenuItem v-slot="{ active }">
              <button @click="sortByOld" class="w-full text-start">
                <span :class="[
                        active ? 'bg-white/25 text-white outline-hidden' : 
                        'text-white', 'block px-4 py-2 text-sm'
                    ]">
